...
B<protocol: PROTO(COL) protocol>. Optionally specify the protocol
...
B<timeout: [WITH] TIMEOUT number SECONDS>. Optionally specifies the
...
B<retry: RETRY number>. Optionally specifies the number of consecutive
...
B<REQUEST> option can set an URI string specifying a document on the
...
B<STATUS> option can be used to explicitly test the HTTP status 
...
B<CHECKSUM> You can test the checksum for documents returned by a HTTP
...
B<HOSTHEADER> option can be used to explicit specify the HTTP
...
Syntax:
 ...
 PROTO(COL) <limit operator number>+

B<limit> is acronym for child status:
 o logging (loglimit)
...
B<operator> is one of "<", "=", ">".

B<number> is percentual numeric limit.
...
B<TARGET> you may specify an alternative recipient for the message,
by adding a valid sip uri after this keyword.

B<MAXFORWARD> Limit the number of proxies or gateways that can forward the
...
B<SECRET> you may specify an alternative secret, default is "testing123".
...
B<HOST> you may specify an alternative Host header

B<REQUEST> you may specify an alternative request, default is "/"

B<ORIGIN> you may specify an alternative origin, default is "http://www.mmonit.com"

B<VERSION> you may specify an alternative version, default is "0"
