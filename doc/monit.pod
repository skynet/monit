...
The I<md5sum> allows to check a certificate checksum. You can use an openssl
CLI to get the checksum value for your certificate, which you can use in Monit's
control file:

 openssl x509 -fingerprint -md5 -in server.crt | head -1 | cut -f2 -d'=' | sed -e 's/://g'
