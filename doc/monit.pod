...
the command-line using C<monit procmatch "regex-pattern">. This will
...
is 600 seconds (5 minutes). The output of the program is recorded and
made available in the User Interface and in alerts (up to 1kB).

=item 9. CHECK NETWORK <unique name> <ADDRESS <ipaddress> | INTERFACE <name>>
...
Note that HTTP support is required for almost all Monit CLI interface
operation, as CLI commands (such as "monit status") are handled by
communicating with the Monit background process via the the HTTP
interface. So basically you should have this enable, though you can
bind the HTTP interface to localhost only so Monit is not accessible
from the outside.
...
B<ALLOW> option which is used to specify authentication and authorize
...
Monit will raise an alert in the following situations:
 ...
 o A network link speed changed
...
The I<from:> option is the sender's email address. That is, the email
...
Services are checked in regular intervals by the C<set daemon n>
statement. Checks are performed in the same order as they are written
in the C<.monitrc> file, except if dependencies are setup between
services.
...
=item 1. custom interval based on a poll cycle multiple

 EVERY [number] CYCLES

=item 2. check is schedule based on a cron-style string

 EVERY [cron]

=item 3. do-not-check schedule based on a cron-style string
...
constraint with the I<every cron> statement, Monit will check whether
...
Note:
the I<status> and I<summary> commands does not support the -g
...
In I<active> mode (the default), Monit will pro-actively monitor a
service and in case of problems raise alerts and/or restart
the service.

In I<passive> mode, Monit will passively monitor a service and will
raise alerts, but will B<not> try to fix a problem by executing
I<start>, I<stop> or I<restart>.

In I<manual> mode, Monit will enter I<active> mode I<only> if a
...
The second format is more advanced and allows to tolerate intermitent
...
"CPU([user|system|wait])", "MEMORY", "SWAP", "CHILDREN", "TOTAL
MEMORY", "LOADAVG([1min|5min|15min])". Some resource tests can be used
...
=head2 NETWORK LINK SPEED TEST

You can check the network link speed changes. This test may only be
used within a check network service entry in the Monit control file.

Syntax:

 IF CHANGED LINK THEN action

I<action> is a choice of "ALERT", "RESTART", "START", "STOP",
"EXEC" or "UNMONITOR".

The test will match if the link speed has changed (e.g. speed dropped)
or if the duplex mode has changed.

NOTE: not all interface types allow for speed monitoring. Pseudo
...
NOTE: as this test depends on the link speed being available, not all
interface types allow to test the saturation. See the LINK SPEED test
description.
...
Crrent upload bandwidth rate test syntax:
 ...
 IF UPLOADED operator value unit PER SECOND THEN action

Current download bandwidth rate test syntax:

 IF DOWNLOADED operator value unit PER SECOND THEN action

Total upload test syntax:

 IF TOTAL UPLOADED operator value unit IN LAST number timeunit THEN action

Total download test syntax:

 IF TOTAL DOWNLOADED operator value unit IN LAST number timeunit THEN action
...
I<timeunit> is a choice of "MINUTE(S)", "HOUR(S)", "DAY".
NOTE: Monit keeps total upload/download statistics only for last
...
Examples:
 ...
 if uploaded > 500 kilobytes per second then alert
...
Crrent upload bandwidth rate test syntax:
 ...
 IF UPLOAD PACKETS operator value PER SECOND THEN action

Current download bandwidth rate test syntax:

 IF DOWNLOAD PACKETS operator value PER SECOND THEN action

Total upload test syntax:

 IF TOTAL UPLOAD PACKETS operator value IN LAST number timeunit THEN action

Total download test syntax:

 IF TOTAL DOWNLOAD PACKETS operator value IN LAST number timeunit THEN action
...
Examples:
        if upload packets > 1000 per second then alert
        if total upload packets > 900000 in last hour then alert
...
automatically, but for security reasons only TLS is used, not SSL. If
you need SSLv2 or SSLv3 you have to explicitly select that method using
I<SSLV2> or I<SSLV3> option.
