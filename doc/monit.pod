...
either on TCP port or unix socket (only one type is supported for Monit
HTTP currently, Monit cannot listen on both TCP port and unix socket at
the same time).
...
If TCP port is used instead of unix socket, you can use your web browser
tp connect to simple HTTP GUI too.

Note that if the HTTP support is disabled the Monit CLI interface won't
work, as CLI commands (such as "monit status") are handled by communicating
with the Monit background process via the the HTTP interface. So basically
you should have this enabled, though you can bind the HTTP interface to
localhost only or use unix socket so Monit is not accessible from the outside.
...
B<UNIXSOCKET> set the path to unix socket Monit should bind to and listen
on.
...
to be a real email-address only a proper formatted address.
...
This test is always active for the process, file, directory and fifo
checks and is if not defined, it defaults to a restart action.
...
Filesystems usually have some space reserved for the root user (ca. 1-5%),
so non-superusers cannot write to a nearly full filesystem. If you set a
limit for the filesystem which is used by non-root users you should reflect
the reserved blocks (Monit reports the reserved blocks percent in the CLI
status and HTTP interface for the given filesystem).
...
utilization based on the current transfer rate vs. link capacity.
...
This example shows a different way to check a web-server using
...
Copyright (C) 2001-2015 by Tildeslash Ltd. All Rights Reserved.
