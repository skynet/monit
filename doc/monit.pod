...
virtual hosts. Examples:
  ...
  if failed host 192.168.1.100 port 8080
     protocol http with status < 400
     and request '/testing/' hostheader 'example.com' 
     with content = "regular.*expression"
     and with timeout 20 seconds for 2 cycles
  then alert

The STATUS option allows to set custom status. By default error
is returned if status code is greater or equal to 400. You can
override this behavior using, for example to test page that
MUST NOT exist (404 should be returned in such case):
...
An example:
 ...
 check program myscript with path /usr/local/bin/myscript.sh with timeout 500 seconds
...
Notes: If the program is a script, the interpreter (first line) is
required. The program or script must also be executable.
