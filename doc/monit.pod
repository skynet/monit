...
//FIXME: document that when hostname is used, we try all options until first success or all failed => the test may hang for timeout and it could be better to use specific IP address (IPv6 or IPv4)
