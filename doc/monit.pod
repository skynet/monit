...
I<PASSWORD> MySQL password.

The credentials are B<optional>. If no credentials are set, Monit will perform
the test using anonymous login. That may cause authentication error to be logged
in your MySQL log for higher MySQL log levels.
...
For example:
 ...
 check process mysql with pidfile /var/run/mysqld/mysqld.pid
     ...
     if failed port 3306 protocol mysql username foo password bar then alert
