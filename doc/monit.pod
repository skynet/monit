...
Monit will raise an event in the following situations:
 ...
 o A process' PID changed out of Monit control
...
Monit provides a default message format. Sample default-formatted message:

 From: monit@foo.bar
 Subject: Monit alert -- Does not exist apache 
 To: admin@foo.bar
 Date: Thu, 04 Sep 2014 02:33:03 +0200
...
You can set custom message format using I<set main-format> statement:
...
optional statements in [brackets]):
 ...
 SET MAILSERVER <hostname | ip-address>
                [PORT number]
                [USERNAME string]
                [PASSWORD string]
                [using SSLV2|SSLV3|TLSV1]
                [CERTMD5 checksum]
                ... 
...
for setting several mail servers:
 ...
 set mailserver
     mail.tildeslash.com,
     mail.foo.bar port 10025 username "myuser" password "mypassword" using tlsv1,
     localhost
...
Monit can monitor the owner user id (uid) of a file object or
process. This test may only be used within following checks:
 o file
 o fifo
 o directory
 o filesystem
 o process (supports also EUID monitoring for effective UID)

The syntax for the uid statement is:

=over 4

=item IF FAILED [E]UID user [[<X>] <Y> CYCLES] THEN action
      [ELSE IF SUCCEEDED [[<X>] <Y> CYCLES] THEN action]
...
Monit can monitor the owner group id (gid) of file objects or
process. This test may only be used within following checks:
 o file
 o fifo
 o directory
...
=back
 ...
 PROTO(COL) HTTP [REQUEST {"/path"} [STATUS operator number]
 [with CHECKSUM checksum] [with HOSTHEADER "string"]
 [CONTENT {= | != } STRING]
...
The STATUS qualifier can be use to explicit test the HTTP status 
...
You can override this behaviour by using the I<status> qualifier, for
example to test that a page does B<not> exist (404 should be returned
in this case):
...
entry it's the I<address> of the remote host. Monit will perform
...
seconds the test will fail and an alert message is sent.
  ...
  if failed 
     port 80 with timeout 15 seconds 
  then alert
...
works. For example, the following is used to test a http server:
 ...
 if failed 
    host www.tildeslash.com port 80 proto http
 then restart
...
You can override default Host header in the HTTP request:

 if failed 
    host 192.168.1.100 port 80 protocol http
    hostheader "example.com"
 then alert
...
Monit will compute a checksum (either MD5 or SHA1 depending
on length of the hash) for the document and compare the computed
checksum with the expected checksum. If the sums does not match 
then the if-tests action is performed, in this case alert. Note
that Monit will B<not> test the checksum for a document if the 
server does not set the HTTP I<Content-Length> header. A HTTP 
server should set this header when it server a static document 
...
strings. Here we show how to test a SMTP server using send and 
expect:
...
=head3 Testing the SIP protocol 
...
the connection type. 
 ...
 check host fwd.pulver.com with address fwd.pulver.com
   if failed 
      port 5060 type tcpssl protocol SIP
...
=head3 Testing the RADIUS protocol
...
=head3 Testing the WEBSOCKET protocol

The WEBSOCKET test is similar to the other protocol tests, but in
addition allows extra optional parameters.

=over 4

=item  IF FAILED [host] [port] [type] PROTOCOL websocket
         [REQUEST string]
         [HOST string]
         [ORIGIN string]
         [VERSION number]
...
HOST:
you may specify an alternative Host header

REQUEST:
you may specify an alternative request, default is "/"

ORIGIN:
you may specify an alternative origin, default is "http://www.mmonit.com"

VERSION:
you may specify an alternative version, default is "0"
...
=item IF STATUS operator value [[<X>] <Y> CYCLES] THEN action
      [ELSE IF SUCCEEDED [[<X>] <Y> CYCLES] THEN action]
...
Multiple status tests can be used, for example:
 ...
 check program hwtest with path /usr/local/bin/hwtest.sh
       ...
       if status = 3 
          for 5 cycles 
       then exec "/usr/local/bin/emergency.sh"
...
insensitive.
 ...
 stop            The program used to stop the specified
                 service. Full path is required. This 
                 statement is optional, but recommended.
 restart         The program used to restart the specified
                 service. Full path is required. This 
                 statement is optional. If not used, Monit will
                 call the service's stop program and then
                 its start program to restart the service.
 ...
 uid, euid       These keywords are either 1) an optional part of
 gid             a start, stop or exec statement. They may be
                 ...
                 used as standalone statements to test a uid
                 and gid attributes.
...
I<pidfile>, I<statefile>, I<group>, I<start>, I<stop>, I<(e)uid>,
...
I<credentials>, I<fips>, I<status>, I<uptime>, I<websocket>,
I<origin>, I<version> and I<failed>
...
page at www.sol.no/server-status with this special protocol test:
 ...
 check process apache with pidfile /var/run/httpd.pid
       ...
       stop  "/etc/init.d/httpd stop"
       if failed 
          host www.sol.no port 80 and use
