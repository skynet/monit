...
status") need to communicate with the Monit background process via the
...
By default Monit will try to ping any available interface (IPv4 or IPv6).
If multiple options are possible (multiple IPv4 and IPv6 addresses)
and a ping via one interface type failed, Monit will try the
next option until connection succeeded or no options are left.
You can force Monit to ping IPv4 or IPv6 address only to make sure the
system is accessible via given IP version by using either PING4 or
PING6 test.
...
Example:
 ...
 check host mmonit.com with address mmonit.com
       if failed ping then alert  # Use any available address (IPv4 or IPv6)

or test that the system is accessible via both IPv4 and IPv6:

 check host mmonit.com with address mmonit.com
       if failed ping4 then alert  # Check IPv4
       if failed ping6 then alert  # Check IPv6
...
I<ipversion: IPV4 | IPV6 >. Optionally specify the IP version
Monit should use when trying to connect to the port. If not used,
Monit will try to connect via any available interface (IPv4 or IPv6).
If multiple options are possible (multiple IPv4 and IPv6 addresses)
and the connection via one interface type failed, Monit will try the
next option until connection succeeded or no options are left.
