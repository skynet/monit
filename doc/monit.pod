...
Monit can perform network ping testing by sending ICMP echo
request datagram packets to a host and check for reply. This test can
only be used within a check host statement. Monit must also run as
root user in order to be able to perform the ping test (requires
raw socket privileges which is usually allowed only to super user).
...
was received, the test succeed. Monit will, by default, send I<three>
ping request packets in one cycle to prevent false alarm (i.e. up to
66% packet loss is tolerated). You can set the B<COUNT> option to a
value between 1 and 20 to send more or less packets. If you require
100% ping success, set the count to 1 (i.e. just one request will be
sent, and if the packet was lost an error will be reported).
...
or with all parameters. Send 5 ping packets to mmonit.com
and wait up to 10 seconds for a reply
